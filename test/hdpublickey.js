...
describe('HDPublicKey interface', function() {
  ...
  describe('derivation', function() {
    it('derivation is the same whether deriving with number or string', function() {
      var pubkey = new HDPublicKey(xpubkey);
      var derived1 = pubkey.getChild(0).getChild(1).getChild(200000);
      var derived2 = pubkey.getChild('m/0/1/200000');
    ...
    it('allows special parameters m, M', function() {
      var expectDerivationSuccess = function(argument) {
        new HDPublicKey(xpubkey).getChild(argument).xpubkey.should.equal(xpubkey);
      };
    ...
    it('can\'t derive hardened keys', function() {
      expectFail(function() {
        return new HDPublicKey(xpubkey).getChild(HDPublicKey.Hardened);
      }, hdErrors.InvalidIndexCantDeriveHardened);
    });

    it('can\'t derive hardened keys via second argument', function() {
      expectFail(function() {
        return new HDPublicKey(xpubkey).getChild(5, true);
      }, hdErrors.InvalidIndexCantDeriveHardened);
    });
    ...
    it('should use the cache', function() {
      var pubkey = new HDPublicKey(xpubkey);
      var derived1 = pubkey.getChild(0);
      var derived2 = pubkey.getChild(0);
