 ...
 * @property {number} newPubKeySize - length of the new public key
 * @property {Buffer} newPubKey
 * @property {number} payloadSigSize - length of the signature
 * @property {Buffer} payloadSig - signature of most recent pubkey
 */
...
SubTxResetKeyPayload.fromBuffer = function (rawPayload) {
  ...
  payload.creditFee = payloadBufferReader.readUInt16LE();
  payload.newPubKeySize = payloadBufferReader.readUInt16LE();
  payload.newPubKey = payloadBufferReader.read(payload.newPubKeySize).reverse().toString('hex');
  payload.payloadSigSize = payloadBufferReader.readUInt16LE();
...
SubTxResetKeyPayload.prototype.validate = function() {
  ...
  Preconditions.checkArgument(BufferUtil.isBuffer(this.newPubKey), 'expect pubKeyId to be a Buffer but got ' + typeof this.newPubKey);
  ...
  Preconditions.checkArgument(this.payloadSig.length === constants.COMPACT_SIGNATURE_SIZE * 2, 'Invalid payloadSig size');
  Preconditions.checkArgument(BufferUtil.isBuffer(this.payloadSig), 'expect payloadSig to be a Buffer but got ' + typeof this.payloadSig);
  Preconditions.checkArgument(this.payloadSig.length === constants.COMPACT_SIGNATURE_SIZE * 2, 'Invalid payloadSig size');
  return true;
};
...
SubTxResetKeyPayload.prototype.toJSON = function toJSON(options) {
  ...
  if (!skipSignature) {
    payloadJSON.payloadSig = this.payloadSigSize;
 ...
 * @return {Buffer}
 */
SubTxResetKeyPayload.prototype.toBuffer = function toBuffer() {
  this.validate();
  ...
  payloadBufferWriter.writeUInt16LE(this.creditFee);
  payloadBufferWriter.writeUInt16LE(this.newPubKeySize);
  payloadBufferWriter.write(this.newPubKey);
  payloadBufferWriter.writeUInt16LE(this.payloadSigSize);
  payloadBufferWriter.write(this.payloadSig);
 ...
 * @return {SubTxResetKeyPayload}
 */
SubTxResetKeyPayload.prototype.copy = function copy() {
  return SubTxResetKeyPayload.fromJSON(this.toJSON());
};
