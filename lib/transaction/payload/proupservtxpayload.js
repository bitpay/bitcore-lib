 ...
 * @property {string} ipAddress
 * @property {number} port
 * @property {Script} scriptOperatorPayout
 * @property {string} inputsHash
 * @property {string} payloadSig
 */
function ProUpServTxPayload(payloadJSON) {
  ...
  if (payloadJSON) {
    ...
    this.scriptOperatorPayout = new Script(payloadJSON.scriptOperatorPayout);
...
ProUpServTxPayload.prototype.validate = function() {
  ...
  var payoutAddress = Address.fromScript(new Script(this.scriptOperatorPayout));
  Preconditions.checkArgument(payoutAddress.isPayToPublicKeyHash() || payoutAddress.isPayToScriptHash(), 'Expected scriptOperatorPayout to be a p2pkh/p2sh');
