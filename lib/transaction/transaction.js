...
Transaction.prototype.toBufferWriter = function(writer) {
  ...
  if (this.isSpecialTransaction() && this.hasExtraPayload()) {
    var payload = payloadParser.parsePayloadBuffer(this.type, this.extraPayload);
...
Transaction.prototype.fromBufferReader = function(reader) {
  ...
  if (this.isSpecialTransaction() && !reader.finished()) {
    ...
    if (extraPayloadSize > 0) {
      var payloadBuffer = reader.read(this.extraPayloadSize);
...
Transaction.prototype.fromObject = function fromObject(arg) {
  ...
  if (transaction.type) {
    ...
    this.setExtraPayload(payloadParser.parsePayloadJSON(transaction.extraPayload))
  }
