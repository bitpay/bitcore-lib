...
//Transaction.prototype.toBuffer = function(includeWitness=true) {
Transaction.prototype.toBuffer = function(includeWitness) {
  var writer = new BufferWriter();
  return this.toBufferWriter(writer, includeWitness).toBuffer();
};

Transaction.prototype.toBufferWriter = function(writer, includeWitness) {
  writer.writeUInt16LE(this.version);
  writer.writeVarintNum(this.nLockTime);
  ...
  var i, k, sizeTxIns = this.inputs.length;
  for (i = 0; i < sizeTxIns; i++)
  {
    var sizeStack = this.inputs[i].witnessStack.length;
    writer.writeVarintNum(sizeStack);
    
    for (k = 0; k < sizeStack; k++)
    {
      var sizeData = this.inputs[i].witnessStack[k].length;
      writer.writeVarintNum(sizeData);
      writer.write(this.inputs[i].witnessStack[k]);
    }
  }
...
Transaction.prototype.fromBufferReader = function(reader) {
  ...
  var i, k, sizeTxIns, sizeTxOuts;

  this.version = reader.readUInt16LE();
  this.nLockTime = reader.readVarintNum();
  ...
  for (i = 0; i < sizeTxIns; i++) {
    this.inputs[i].witnessStack = [];
    var sizeStack = reader.readVarintNum();
    
    for (k = 0; k < sizeStack; k++)
    {
      var sizeData = reader.readVarintNum();
      var witnessData;
      if (sizeData !== 0)
        witnessData = reader.read(sizeData);
      else
        witnessData = new buffer.Buffer([]);
      this.inputs[i].witnessStack.push(witnessData);
    }
  }
