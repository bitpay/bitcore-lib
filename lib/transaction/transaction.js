...
Transaction.prototype.getSerializationError = function(opts) {
  ...
  if (this.isSpecialTransaction() && !this.hasCorrectExtraPayloadSize()) {
...
Transaction.prototype.toBufferWriter = function(writer) {
  ...
  if (this.isSpecialTransaction() && this.hasCorrectExtraPayloadSize()) {
    this.setExtraPayload(this.payload.toBuffer());
    writer.writeVarintNum(this.extraPayloadSize);
    writer.write(this.extraPayload);
  }
  return writer;
};
...
Transaction.prototype.fromBufferReader = function(reader) {
  ...
  if (this.isSpecialTransaction() && !reader.finished()) {
    ...
    if (extraPayloadSize > 0) {
      ...
      this.extraPayload = reader.read(this.extraPayloadSize);
      this.payload = payloadParser.parsePayload(this.type, this.extraPayload);
    }
  }
...
Transaction.prototype.toObject = Transaction.prototype.toJSON = function toObject() {
  ...
  if (this.isSpecialTransaction()) {
    obj.type = this.type;
    obj.extraPayloadSize = this.extraPayloadSize;
    obj.extraPayload = this.extraPayload;
    obj.payload = payloadParser.serializePayloadToJson(this.payload);
  }
  return obj;
};
...
Transaction.prototype.fromObject = function fromObject(arg) {
  ...
  if (transaction.type) {
    ...
    this.extraPayloadSize = transaction.extraPayloadSize;
...
Transaction.prototype._checkConsistency = function(arg) {
  ...
  if (this.isSpecialTransaction()) {
    $.checkState(this.hasCorrectExtraPayloadSize(), 'Special transaction payload size is incorrect');
  }
};
...
Transaction.prototype.hasExtraPayload = function() {
  return this.extraPayloadSize > 0 && BufferUtil.isBuffer(this.extraPayload);
};

/**
 * Checks if extra payload size is correct
 * @returns {boolean}
 */
Transaction.prototype.hasCorrectExtraPayloadSize = function() {
  return this.hasExtraPayload() && this.extraPayloadSize === this.extraPayload.length;
};
 ...
 * @returns {Transaction}
 */
Transaction.prototype.setExtraPayload = function(extraPayload) {
