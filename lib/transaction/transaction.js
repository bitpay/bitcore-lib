...
var PayloadFactory = require('./payload/payload');
var payloadConstants = require('./payload/constants');
var registeredTransactionTypes = payloadConstants.registeredTransactionTypes;
...
Transaction.TYPES = payloadConstants.registeredTransactionTypes;
...
Transaction.prototype.toBufferWriter = function(writer) {
  ...
  if (this.isSpecialTransaction() && this.hasExtraPayload()) {
    var payload = PayloadFactory.serializePayloadToBuffer(this.extraPayload);
...
Transaction.prototype.fromBufferReader = function(reader) {
  ...
  if (this.isSpecialTransaction() && !reader.finished()) {
    ...
    if (extraPayloadSize > 0) {
      ...
      this.setExtraPayload(PayloadFactory.parsePayloadBuffer(this.type, payloadBuffer));
    }
  }
...
Transaction.prototype.toObject = Transaction.prototype.toJSON = function toObject() {
  ...
  if (this.isSpecialTransaction()) {
    obj.type = this.type;
    obj.extraPayload = PayloadFactory.serializePayloadToJSON(this.extraPayload);
  }
  return obj;
};
...
Transaction.prototype.fromObject = function fromObject(arg) {
  ...
  if (transaction.type) {
    ...
    this.setExtraPayload(PayloadFactory.parsePayloadJSON(transaction.type, transaction.extraPayload))
  }
...
Transaction.prototype.setType = function(type) {
  ...
  this.type = type;
  this.setExtraPayload(PayloadFactory.createPayload(type));
...
Transaction.prototype.setExtraPayload = function(payload) {
  ...
  if (!PayloadFactory.isPayloadHasCorrectType(this.type, payload)) {
...
Transaction.prototype.getExtraPayloadSize = function getExtraPayloadSize() {
  return PayloadFactory.serializePayloadToBuffer(this.extraPayload).length;
};
