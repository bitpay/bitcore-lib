...
function Transaction(serialized) {
  ...
  if (serialized) {
    ...
    } else if (JSUtil.isObject(serialized)) {
...
Transaction.prototype.lockUntilBlockHeight = function(height) {
  $.checkArgument(typeof height === 'number');
...
Transaction.prototype._removeOutput = function(index) {
  this.outputs = this.outputs.filter(function(output, outputIndex){
...
Transaction.prototype._newOutputOrder = function(newOutputs) {
  var isInvalidSorting = (this.outputs.length !== newOutputs.length ||
    this.outputs.filter(function(output){ return newOutputs.indexOf(output) < 0}).length !== 0);
  if (isInvalidSorting) {
...
Transaction.prototype.removeInput = function(txId, outputIndex) {
  var index;
  if (!outputIndex && typeof txId === 'number') {
    index = txId;
  } else {
  ...
  this.inputs = this.inputs.filter(function(input, inputIndex){
...
Transaction.prototype.sign = function(privateKey, sigtype) {
  ...
  this.getSignatures(privateKey, sigtype).forEach(function(signature) {
...
Transaction.prototype.getSignatures = function(privKey, sigtype) {
  ...
  this.inputs.forEach(function forEachInput(input, index) {
    input.getSignatures(transaction, privKey, index, sigtype, hashData).forEach(function(signature) {
...
Transaction.prototype.verifySignature = function(sig, pubkey, nin, subscript, sigversion, satoshis) {
  ...
  if (typeof sigversion === 'undefined') {
    sigversion = 0;
  }
