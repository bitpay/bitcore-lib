...
function addNetwork(data) {
  ...
  for (var entry of Object.entries(network)) {
    if (entry[1]) {
      networkMaps[entry[1]] = network;
    }
  }
...
for (var key in TESTNET) {
  if (!JSUtil.isObject(TESTNET[key])) {
...
for (var key in REGTEST) {
  if (!JSUtil.isObject(REGTEST[key])) {
